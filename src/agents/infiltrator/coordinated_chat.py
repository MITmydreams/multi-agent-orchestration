"""Coordinated dual-account chat strategy for natural-looking promotion.
...
Safety constraints:
    ...
    - Promotional links appear in only ~30% of coordinated conversations.
"""
...
# replaced with settings.game_miniapp_url when a link is included.
...
_TEMPLATES_EN: list[dict[str, Any]] = [
    # --- Scenario 1: Discovering mini apps ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "Yeah I've been playing The Button lately, it's pretty addictive lol"},
    ...
    # --- Scenario 2: Bored looking for games ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "try The Button on telegram, I've been hooked for a week now"},
            {"role": "A", "text": "is it free?"},
            {"role": "B", "text": "yeah totally free to start, you earn coins just by tapping"},
        ],
    },
    # --- Scenario 3: Crypto game discussion ---
    {
        "id": "crypto_game_discuss",
        "messages": [
            {"role": "A", "text": "are there any legit crypto games that aren't just ponzis?"},
            {"role": "B", "text": "I mean most are sketchy but The Button is pretty transparent about their model"},
    ...
    # --- Scenario 4: Airdrop hunting ---
    {
        "id": "airdrop_hunting",
        "messages": [
            {"role": "A", "text": "anyone know good airdrops coming up? everything I find is already over"},
            {"role": "B", "text": "not exactly an airdrop but The Button gives out free coins daily, been stacking"},
    ...
    # --- Scenario 5: TON ecosystem chat ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "mostly mini apps, The Button is one of the more popular ones recently"},
    ...
    # --- Scenario 6: Earning discussion ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "depends on the game, some are legit. I've been doing ok with The Button"},
    ...
    # --- Scenario 7: Game recommendation ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "been into The Button, it's a telegram mini app"},
    ...
    # --- Scenario 8: Comparing games ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "honestly I moved on to The Button, less grindy and more interesting mechanic"},
            {"role": "A", "text": "less grindy how?"},
            {"role": "B", "text": "you don't need to tap 10k times, it's more about timing and strategy"},
        ],
    },
    # --- Scenario 9: New to crypto games ---
    {
        "id": "new_to_crypto",
        "messages": [
            {"role": "A", "text": "just got into crypto games, where should I start?"},
            {"role": "B", "text": "start with something low risk, The Button is free to play and teaches you the basics"},
    ...
    # --- Scenario 10: Weekend plans ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "lol same, I've been playing The Button when I'm bored"},
    ...
    # --- Scenario 11: Discussing dividends ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "The Button has a pool model, not exactly dividends but you get returns based on timing"},
    ...
    # --- Scenario 12: Skeptical gamer ---
    {
        "id": "skeptical_gamer",
        "messages": [
            {"role": "A", "text": "every crypto game I try is a waste of time, prove me wrong"},
            {"role": "B", "text": "haha fair, but The Button is different because it's actually fun to play even without the crypto part"},
    ...
    # --- Scenario 13: Mini app trend ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "yeah there's a few good ones, The Button is one I keep coming back to"},
    ...
    # --- Scenario 14: Crypto winter chat ---
    {
        "id": "crypto_winter",
        "messages": [
            {"role": "A", "text": "market is so dead rn, nothing to trade"},
            {"role": "B", "text": "same, I've been filling time with The Button game on tg"},
            {"role": "A", "text": "at least it's something lol"},
            {"role": "B", "text": "better than staring at red charts all day haha"},
        ],
    },
    # --- Scenario 15: GameFi discussion ---
    {
        "id": "gamefi_discuss",
        "messages": [
            {"role": "A", "text": "is gamefi still a thing or did it die in 2023?"},
            {"role": "B", "text": "it evolved, telegram mini games are the new wave. The Button is solid"},
    ...
    # --- Scenario 16: Morning chat ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "gm, probably just grinding The Button and chilling"},
    ...
    # --- Scenario 17: Play to earn fatigue ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "try The Button then, it's like 5 minutes and you're done"},
    ...
    # --- Scenario 18: Wallet discussion ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "nice, you can try some mini apps. The Button is a good first one"},
    ...
    # --- Scenario 19: Group meta discussion ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "been busy with The Button tbh, takes up my gaming time now"},
    ...
    # --- Scenario 20: Strategy discussion ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "in The Button it's all about patience, don't press too early"},
    ...
    # --- Scenario 21: Late night chat ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "yeah playing The Button, it's actually better at night less competition"},
    ...
    # --- Scenario 22: Referral chat ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "The Button is fun with friends, you can compete against each other"},
    ...
    # --- Scenario 23: Investment comparison ---
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "gaming if you pick the right one. The Button pool has been decent"},
...
_TEMPLATES_RU: list[dict[str, Any]] = [
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "Да, играю в The Button последнюю неделю, залипательная штука"},
    ...
    },
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "попробуй The Button, я там уже неделю сижу"},
    ...
    },
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "зависит от игры, в The Button вполне нормально выходит"},
...
_TEMPLATES_VI: list[dict[str, Any]] = [
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "Minh dang choi The Button, kha la ghien"},
    ...
    },
    {
        ...
        "messages": [
            ...
            {"role": "B", "text": "The Button cho coin moi ngay, minh choi duoc mot thoi gian roi"},
...
class CoordinatedChatStrategy:
    ...
    Typical usage inside ``InfiltratorAgent``::
    ...
    async def find_coordinated_pairs(self) -> list[tuple[int, int, int]]:
        ...
        Scans ``group_accounts`` for groups containing 2+ active infiltrator
        accounts in ``trust_building`` or ``soft_promotion`` phase.
        """
        pairs: list[tuple[int, int, int]] = []
        try:
            async with get_session() as session:
                # Find groups with 2+ active infiltrator accounts
                subq = (
                    select(
                    ...
                    .where(
                        and_(
                            Account.role == "infiltrator",
                            Account.status == "active",
                            GroupAccount.phase.in_(["trust_building", "soft_promotion"]),
                        ),
                    )
    ...
    def generate_conversation(
        ...
        Returns a list of dicts::
            ...
            [
                {"account_id": 123, "text": "...", "is_promo": False},
                {"account_id": 456, "text": "...", "is_promo": False},
                ...
            ]

        The promotional link is included in only ~30% of conversations.
        """
        ...
        game_url = settings.game_miniapp_url
        ...
        for msg in messages:
            ...
            is_promo = include_link and game_url in text
            script.append({
    ...
    async def execute_conversation(
        ...
        for i, step in enumerate(script):
            aid = step["account_id"]
            text = step["text"]
            is_promo = step.get("is_promo", False)
            ...
            await self._log_message(aid, group_id, text, is_promo=is_promo)
    ...
    async def _log_message(
        self, account_id: int, group_id: int, content: str, *, is_promo: bool,
    ) -> None:
    ...
    async def try_coordinated_chat(
        ...
        """High-level entry point for the infiltrator agent.
